    ...
    public static void sort(final List<Stock> list) {
        Collections.sort(list, new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                int name = stock1.get("name").toString().compareTo(stock2.get("name").toString());
                if (name != 0) return name;
                else {
                    int date = ((Date) stock1.get("date")).compareTo((Date) stock2.get("date"));
                    if (date != 0) return date;
                    else {
                        Double a;
                        Double b;
                        if (stock1.containsKey("change")){
                            a = ((Double)stock1.get("change"));
                        } else a = ((Double) stock1.get("last")) - ((Double) stock1.get("open"));
                        if (stock2.containsKey("change")){
                            b = ((Double)stock2.get("change"));
                        } else b = ((Double) stock2.get("last")) - ((Double) stock2.get("open"));
                        int change = b.compareTo(a);
